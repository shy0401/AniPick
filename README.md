...
배포 환경은 Synology NAS입니다. NAS 장비가 32-bit ARM 환경이라 Docker와 Prisma Query Engine을 직접 실행할 수 없어, 운영 배포에서는 `backend/src/server-nas.js` 경량 API 서버를 사용합니다.
...
- `/Anipick-api/api/anime/trending?lang=ko`, `lang=en`, `lang=ja` 응답 확인
- `/Anipick-api/api/anime/59845?lang=ko` 제목이 `향기로운 꽃은 늠름하게 핀다`로 표시됨
- 회원가입/로그인 API 동작 확인
